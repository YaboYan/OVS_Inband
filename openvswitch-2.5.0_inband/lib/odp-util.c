...
static __be16
get_tcp_src(const union ovs_key_tp *tcp_key)
{
	return tcp_key->tcp.tcp_src;
}

__be16
get_tcp_key(const struct nlattr *key[OVS_KEY_ATTR_MAX + 1])
{
	const union ovs_key_tp *tcp_key;
	tcp_key = nl_attr_get(key[OVS_KEY_ATTR_TCP]);
	__be16 tcp_src=get_tcp_src(tcp_key);
	return tcp_src;
}
