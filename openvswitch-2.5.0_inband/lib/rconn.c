...
int rconn_send_with_limit(struct rconn *rc, struct ofpbuf *b,
                          ...
                          int queue_limit)
    OVS_EXCLUDED(rc->mutex);
int rconn_send_with_limit_kernal(struct rconn *rc, struct ofpbuf *b,
        struct rconn_packet_counter *counter, int queue_limit,const char *type,unsigned int src_ipv4,
		  unsigned int dst_ipv4, struct eth_addr dst_ea, struct eth_addr src_ea, unsigned int port_id,struct dpif *dpif,const char *br_name)
    OVS_EXCLUDED(rc->mutex);
...
unsigned int rconn_count_txqlen(const struct rconn *rc)
    OVS_EXCLUDED(rc->mutex);
void rconn_update_version(struct rconn *rconn, uint32_t version);
void rconn_update_state(struct rconn *rconn,struct rconn *rconn_src);
void vconn_create_and_update(struct rconn *rconn,struct rconn *rconn_org);
void rconn_fake_init(struct rconn *rconn);
void
vlog_out(struct rconn *rconn)
{
	 VLOG_INFO("ofconn_run,rconn_name: %s,mutex:%s",rconn->name,rconn->mutex.where);
}

void
rconn_update_version(struct rconn *rconn, uint32_t version)

{
	ovs_mutex_lock(&rconn->mutex);
	rconn->allowed_versions=version;
	ovs_mutex_unlock(&rconn->mutex);
}
void
rconn_update_state(struct rconn *rconn,struct rconn *rconn_src)

{
	ovs_mutex_lock(&rconn->mutex);
	rconn->state=rconn_src->state;
	ovs_mutex_unlock(&rconn->mutex);
}
void
vconn_create_and_update(struct rconn *rconn,struct rconn *rconn_org)
{
	ovs_mutex_lock(&rconn->mutex);
	if(rconn->vconn)
   rconn->vconn=vconn_alloc();
   vconn_update(rconn->vconn,rconn_org->vconn);
   ovs_mutex_unlock(&rconn->mutex);
}
void
rconn_fake_init(struct rconn *rconn)
{
	ovs_mutex_lock(&rconn->mutex);
	if(!rconn->vconn)
	  rconn->vconn=vconn_alloc();
	rconn->state=8;
	vconn_set(rconn->vconn);
	ovs_mutex_unlock(&rconn->mutex);
}
bool
run_valid_rc(struct rconn *rconn)
{
	if(rconn->mutex.where)
	{
//		VLOG_INFO("true");
		return true;
	}
...
run_BACKOFF(struct rconn *rc)
    OVS_REQUIRES(rc->mutex)
{
    if (timed_out(rc)&&rc->operate!=false) {
        reconnect(rc);
    }
}
...
static int
rconn_send__fake(struct rconn *rc, struct ofpbuf *b,
           struct rconn_packet_counter *counter,const char *type,unsigned int src_ipv4,
			  unsigned int dst_ipv4, struct eth_addr dst_ea, struct eth_addr src_ea, unsigned int port_id,struct dpif *dpif,const char *br_name)
    OVS_REQUIRES(rc->mutex)
{
...
/*        list_push_back(&rc->txq, &b->list_node);*/
         ...
         * vconn is backlogged and there's no point in stuffing more into it
         * now.  We'll get back to that in rconn_run().) */
        dpif_ofbuf_send_back(type,b, src_ipv4,dst_ipv4,port_id,dpif,src_ea,dst_ea,br_name);
        if (counter) {
 ...
 * effect of waking up poll_block(). */
int
rconn_send_with_limit(struct rconn *rc, struct ofpbuf *b, struct rconn_packet_counter *counter, int queue_limit)
    OVS_EXCLUDED(rc->mutex)
{
    int error;
...
int
rconn_send_with_limit_kernal(struct rconn *rc, struct ofpbuf *b,
        struct rconn_packet_counter *counter, int queue_limit,const char *type,unsigned int src_ipv4,
		  unsigned int dst_ipv4, struct eth_addr dst_ea, struct eth_addr src_ea, unsigned int port_id,struct dpif *dpif,const char *br_name)
OVS_EXCLUDED(rc->mutex)
{
    ...
    ovs_mutex_lock(&rc->mutex);
    if (rconn_packet_counter_n_packets(counter) < queue_limit) {
        error = rconn_send__fake(rc, b, counter,type,src_ipv4,dst_ipv4,dst_ea,src_ea,port_id,dpif,br_name);
    } else {
        COVERAGE_INC(rconn_overflow);
        ofpbuf_delete(b);
        error = EAGAIN;
...
void
rconn_set_target(struct rconn *rc, const char *target, const char *name)
OVS_REQUIRES(rc->mutex)
{
  ovs_mutex_lock(&rc->mutex);
  free(rc->name);
