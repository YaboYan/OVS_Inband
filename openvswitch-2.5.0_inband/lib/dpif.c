...
void dpif_ofbuf_send(const char *type,struct ofpbuf *of_msg,unsigned int src_ip,
		unsigned int dst_ip,  ovs_be16 port_id, struct dpif *dpif,struct eth_addr src_ea,struct eth_addr dst_ea,const char *br_name)
{
	struct registered_dpif_class *registered_class;
	registered_class = dp_class_lookup(type);
	registered_class->dpif_class->dpif_netlink_ofbuf_send(of_msg ,dpif ,src_ip,
			dst_ip,port_id,src_ea,dst_ea,br_name);
}
void dpif_ofbuf_send_back(const char *type,struct ofpbuf *of_msg,unsigned int src_ip,
		unsigned int dst_ip,  ovs_be16 port_id, struct dpif *dpif,struct eth_addr src_ea,struct eth_addr dst_ea,const char *br_name)
{
	struct registered_dpif_class *registered_class;
	registered_class = dp_class_lookup(type);
	registered_class->dpif_class->dpif_netlink_ofbuf_send(of_msg ,dpif ,dst_ip,
			src_ip,port_id,dst_ea,src_ea,br_name);
}
static void
dp_initialize(void)
{
...
static int
do_open_ex(const char *name, const char *type, bool create, struct dpif **dpifp, __be32  ipv4)
{
    ...
    error = registered_class->dpif_class->open_ex(registered_class->dpif_class,
                                               name, create, &dpif, ipv4);
    if (!error) {
...
int
dpif_open_ex(const char *name, const char *type, struct dpif **dpifp, __be32  ipv4)
{
    return do_open_ex(name, type, false, dpifp, ipv4);
}
int
...
int
dpif_create_ex(const char *name, const char *type, struct dpif **dpifp, __be32  ipv4)
{
    return do_open_ex(name, type, true, dpifp,ipv4);
}
int
...
int
dpif_create_and_open(const char *name, const char *type, struct dpif **dpifp , __be32  ipv4)
{
    int error;

    error = dpif_create_ex(name, type, dpifp, ipv4);
    if (error == EEXIST || error == EBUSY) {
        error = dpif_open_ex(name, type, dpifp, ipv4);
        if (error) {
 ...
 * non-null). */
int
dpif_port_add(struct dpif *dpif, struct netdev *netdev, odp_port_t *port_nop,__be32  ipv4)
{
    ...
    error = dpif->dpif_class->port_add(dpif, netdev, &port_no, ipv4);
    if (!error) {
...
dpif_upcall_type_to_string(enum dpif_upcall_type type)
{
    ...
    case DPIF_OFBUF: return "OFBUF";
    case DPIF_BACK: return "C_BACK";
