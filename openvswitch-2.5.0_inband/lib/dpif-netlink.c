...
static int ovs_ofbuf_family;

/* Generic Netlink multicast groups for OVS.
 *
 * Initialized by dpif_netlink_init(). */
static unsigned int ovs_vport_mcgroup;
static void dpif_netlink_ofpbuf_init(struct dpif_netlink_ofpbuf *ofpuf);
static void dpif_netlink_modify_ofpbuf(struct dpif_netlink_ofpbuf *request, struct ofpbuf *buf,struct ofpbuf *bufp_trans,
		unsigned int dst_ip, unsigned int src_ip,ovs_be16 port_id,struct eth_addr src_ea,struct eth_addr dst_ea,const char *br_name);
static int netlink_ofbuf_transact(struct dpif_netlink_ofpbuf *request, struct ofpbuf *bufp_trans,
		unsigned int dst_ip, unsigned int src_ip, ovs_be16 port_id,struct eth_addr src_ea,struct eth_addr dst_ea,const char *br_name);
...
static int dpif_netlink_vport_from_ofpbuf(struct dpif_netlink_vport *,
                                          const struct ofpbuf *);
static void
dpif_netlink_ofpbuf_init(struct dpif_netlink_ofpbuf *ofpuf)
{
    memset(ofpuf, 0, sizeof *ofpuf);
}
static void
dpif_netlink_modify_ofpbuf(struct dpif_netlink_ofpbuf *request, struct ofpbuf *buf,struct ofpbuf *bufp_trans,
	unsigned int dst_ip, unsigned int src_ip, ovs_be16 port_id,struct eth_addr src_ea,struct eth_addr dst_ea,const char *br_name)
{
    struct ovs_header *ovs_header;
    size_t len = bufp_trans->size;
    size_t ea_len=sizeof(struct eth_addr);
    nl_msg_put_genlmsghdr(buf, 0, ovs_ofbuf_family, NLM_F_REQUEST,
    ...
    nl_msg_put_string(buf,OVS_OFBUFF_ATTR_BR_NAME,br_name);
    nl_msg_put_unspec(buf,OVS_OFBUFF_ATTR_DATA, bufp_trans->data,len);
}
static int
netlink_ofbuf_transact(struct dpif_netlink_ofpbuf *request, struct ofpbuf *bufp_trans,
		unsigned int dst_ip,  unsigned int src_ip, ovs_be16 port_id,struct eth_addr src_ea,struct eth_addr dst_ea,const char *br_name)
{
	struct ofpbuf *request_buf;
	int error;
    request_buf = ofpbuf_new(1024);
    dpif_netlink_modify_ofpbuf(request, request_buf, bufp_trans,dst_ip, src_ip, port_id,src_ea,dst_ea,br_name);
...
static void
dpif_netlink_ofbuf_send(struct ofpbuf *ofbuff_trans, struct dpif *dpif_ ,unsigned int src_ip,
		unsigned int dst_ip, ovs_be16 port_id,struct eth_addr src_ea,struct eth_addr dst_ea, const char *br_name)
{
    const struct dpif_netlink *dpif = dpif_netlink_cast(dpif_);
	struct dpif_netlink_ofpbuf request;
	dpif_netlink_ofpbuf_init(&request);
    request.cmd = OVS_OFBUF_SEND;
    request.dp_ifindex = dpif->dp_ifindex;
    netlink_ofbuf_transact(&request, ofbuff_trans, dst_ip, src_ip, port_id,src_ea,dst_ea,br_name);
}
...
static int
dpif_netlink_open_ex(const struct dpif_class *class OVS_UNUSED, const char *name,
                  bool create, struct dpif **dpifp, __be32  ipv4)
{
    struct dpif_netlink_dp dp_request, dp;
    struct ofpbuf *buf;
    uint32_t upcall_pid;
...
dpif_netlink_port_add__(struct dpif_netlink *dpif, struct netdev *netdev,
                        odp_port_t *port_nop,__be32 ipv4)
    OVS_REQ_WRLOCK(dpif->upcall_lock)
{
                      ...
                      odp_port_t *port_nop,__be32  ipv4)
{
    ...
    fat_rwlock_wrlock(&dpif->upcall_lock);
    error = dpif_netlink_port_add__(dpif, netdev, port_nop,ipv4);
    fat_rwlock_unlock(&dpif->upcall_lock);
                          ...
                          const char *port_name, struct dpif_port *dpif_port)
{
    ...
    error = dpif_netlink_vport_transact(&request, &reply, &buf);// 0 success
    if (!error) {
                        ...
                        struct dpif_netlink_flow *reply, struct ofpbuf **bufp)
{
    ...
    dpif_netlink_init_flow_get__(dpif, key, key_len, ufid, terse, &request); //mostly inite request
                 ...
                 struct dpif_upcall *upcall, int *dp_ifindex)
{
    ...
    type = (genl->cmd == OVS_PACKET_CMD_MISS ? DPIF_UC_MISS
            ...
            : genl->cmd==OVS_PACKET_CMD_OFBUF ? DPIF_OFBUF
            : genl->cmd==OVS_PACKET_CMD_BACK ? DPIF_BACK: -1);
    if (type < 0) {
        return EINVAL;
    }
...
const struct dpif_class dpif_netlink_class = {
    "system",
    NULL,                       /* init */
	dpif_netlink_ofbuf_send, /* send ofpuf to data level*/
    dpif_netlink_enumerate,
    NULL,
	dpif_netlink_open_ex,
    dpif_netlink_open,
    dpif_netlink_close,
...
dpif_netlink_init(void)
{
    ...
    if (ovsthread_once_start(&once)) {
        ...
        if(!error){
        	error=nl_lookup_genl_family(OVS_OFBUF_FAMILY,
                    &ovs_ofbuf_family);
        }
        if (!error) {
                             ...
                             struct ofpbuf *buf)
{
    ...
    if (vport->ipv4) {
            nl_msg_put_u32(buf, OVS_VPORT_ATTR_IPV4,
                              vport->ipv4);
        }
...
dpif_netlink_dp_to_ofpbuf(const struct dpif_netlink_dp *dp, struct ofpbuf *buf)
{
    ...
    if (dp->ipv4) {
        nl_msg_put_u32(buf, OVS_DP_ATTR_ipv4, dp->ipv4);
    }
