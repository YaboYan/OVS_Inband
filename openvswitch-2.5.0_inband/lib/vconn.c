...
struct vconn *vconn_alloc(void);
void vconn_set(struct vconn *vconn);
void vconn_update(struct vconn *vconn,struct vconn *vconn_src);
...
{
	struct vconn *vconn;
	vconn=xmalloc(sizeof(struct vconn));
	return vconn;
}
void
vconn_update(struct vconn *vconn,struct vconn *vconn_src)
{
   vconn->allowed_versions=vconn_src->allowed_versions;
   vconn->version=vconn_src->version;
   vconn->peer_versions=vconn_src->peer_versions;
}
void
vconn_set(struct vconn *vconn)
{
	vconn->state=3;
	vconn->version=OFP13_VERSION;
	vconn->vclass=NULL;
	vconn->allowed_versions=-1;
	vconn->peer_versions=-1;
	vconn->error=-1;
}
