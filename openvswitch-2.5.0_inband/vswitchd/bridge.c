...
__be32 ipv4 = 167772285;
__be32 ipv4_lim = 167772414;
ovs_be16 port_id = 81;
struct bridge *br_main;//
struct iface {
...
static void bridge_create(const struct ovsrec_bridge *, __be32 ipv4);
...
static void bridge_configure_remotes(struct bridge *,
                                     ...
                                     size_t n_managers,struct bridge *);
static void bridge_configure_remotes_main(struct bridge *br,
...
bridge_reconfigure(const struct ovsrec_open_vswitch *ovs_cfg)
{
    ...
    HMAP_FOR_EACH_SAFE (br, next, node, &all_bridges) {
        if (!br->ofproto) {
            ...
            error = ofproto_create(br->name, br->type, &br->ofproto, br->ipv4);/////////////
            if (error) {
    ...
    HMAP_FOR_EACH (br, node, &all_bridges) {
        ...
        if (strcmp(br->name, "s1")){
        	bridge_configure_remotes(br, managers, n_managers ,br_main);
        }
        else
        {
    	br_main=br;
        bridge_configure_remotes_main(br, managers, n_managers);
        }
...
bridge_configure_datapath_id(struct bridge *br)
{
    ...
    br->ea = ea;
    if(ea.be16)
    VLOG_INFO("bridge %s: using datapath ID %016"PRIx64, br->name, (uint64_t)ea.be16);
    else
    	VLOG_INFO("bridge %s: using datapath ID %016"PRIx64, br->name, (uint64_t)ea.ea);
...
add_del_bridges(const struct ovsrec_open_vswitch *cfg)
{
    ...
    SHASH_FOR_EACH(node, &new_br) {
        ...
        if (!br) {
        	if (strcmp(br_cfg->name, "s1")){
            bridge_create(br_cfg,ipv4);
            ipv4++;
            if (ipv4 == ipv4_lim)
            {
            	VLOG_WARN("bridge %s ip exceed limits", br_cfg->name);
            }
        	}
        	else
        	{
        		bridge_create(br_cfg,167772284);
        	}
        }
...
static void
bridge_create(const struct ovsrec_bridge *br_cfg,__be32 ipv4)
{
    struct bridge *br;
    ...
    br->cfg = br_cfg;
    br->ipv4=ipv4;
    VLOG_INFO("bridge:%s_use_ip:%u",br->name,br->ipv4);
...
static void
bridge_configure_remotes_main(struct bridge *br,
                         const struct sockaddr_in *managers, size_t n_managers)
{
    ...
    ofproto_set_controllers_main(br->ofproto, ocs, n_ocs,
...
bridge_configure_remotes(struct bridge *br,
                         const struct sockaddr_in *managers, size_t n_managers,struct bridge *br_main )
{
    bool disable_in_band;
    ...
    ofproto_set_controllers(br->ofproto,br_main->ofproto, br->ea, br_main->ea,ocs, n_ocs,
                            bridge_get_allowed_versions(br));
    free(ocs[0].target); /* From bridge_ofproto_controller_for_mgmt(). */
    free(ocs);
