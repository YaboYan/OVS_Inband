...
struct dpif *
dpif_ofproto_cast(const struct ofproto *ofproto)
{
	struct ofproto_dpif * ofproto_=ofproto_dpif_cast(ofproto);
	return ofproto_->backer->dpif;
}
struct ofproto *
ofproto_ofprotodpif_cast(struct ofproto_dpif *ofproto)
{
	return &ofproto->up;
}
...
static int
open_dpif_backer(const char *type, struct dpif_backer **backerp, __be32  ipv4  )
{
    struct dpif_backer *backer;
    ...
    error = dpif_create_and_open(backer_name, type, &backer->dpif, ipv4);
    free(backer_name);
    if (error) {
...
construct(struct ofproto *ofproto_)
{
    ...
    error = open_dpif_backer(ofproto->up.type, &ofproto->backer, ofproto_->ipv4);
    if (error) {
        return error;
    }
...
port_add(struct ofproto *ofproto_, struct netdev *netdev)
{
    ...
    if (!dpif_port_exists(ofproto->backer->dpif, dp_port_name)) {
        ...
        error = dpif_port_add(ofproto->backer->dpif, netdev, &port_no,ofproto_->ipv4);
        if (error) {
            return error;
        }
        if (netdev_get_tunnel_config(netdev)) {
