...
#define N_SCHEDULERS 2
    ...
    struct eth_addr src_ea;//////////////
	struct eth_addr dst_ea;////////////////
	ovs_be16 port_id;//////////////
     char *br_name;
     u32 flag;
    struct ofconn *ofconn_pair;
...
static struct ofconn *ofconn_create_main(struct connmgr *mgr, struct connmgr *mgr_main,struct eth_addr ea,struct eth_addr ea_main,struct rconn *rconn, enum ofconn_type type,
        bool enable_async_msgs,ovs_be16 port_id)
    OVS_REQUIRES(ofproto_mutex);
static struct ofconn *ofconn_create_fake(struct connmgr *mgr, struct connmgr *mgr_main,struct eth_addr ea,struct eth_addr ea_main,struct rconn *rconn, enum ofconn_type type,
        bool enable_async_msgs,ovs_be16 port_id,const char *target,const char *name, struct ofconn *ofconn_pair )
    OVS_REQUIRES(ofproto_mutex);
static struct ofconn *ofconn_create(struct connmgr *mgr, struct rconn *rconn, enum ofconn_type type,
              bool enable_async_msgs,ovs_be16 port_id,u32 flag)
OVS_REQUIRES(ofproto_mutex);
...
static void ofconn_run(struct ofconn *ofconn, char *type,
								   struct dpif *dpif);
...
void rconn_update(struct ofconn *ofconn);
...
u32
ofconn_flag(struct ofconn *ofconn)
{
	return ofconn->flag;
}
struct ofconn *
port_id_get_ofconn(struct connmgr *mgr,ovs_be16 tcp_src)
{
	struct ofconn *ofconn_find;
	LIST_FOR_EACH (ofconn_find, node, &mgr->all_conns) {
	    if(ofconn_find->port_id==tcp_src)
         return ofconn_find;
	}
	return NULL;
}
...
void
rconn_update(struct ofconn *ofconn)
{
	int version;
	version=rconn_get_version(ofconn->ofconn_pair->rconn);
	rconn_update_version(ofconn->rconn,version);
	rconn_update_state(ofconn->rconn,ofconn->ofconn_pair->rconn);
	vconn_create_and_update(ofconn->rconn,ofconn->ofconn_pair->rconn);
...
connmgr_create(struct ofproto *ofproto,
               const char *name, const char *local_port_name,__be32  ipv4)
{
    struct connmgr *mgr;
...
void
connmgr_run(struct connmgr *mgr, char *type,
									struct dpif *dpif)
    ...
    HMAP_FOR_EACH (ofservice, node, &mgr->services) {
        ...
        if (!retval) {
            ...
            ofconn = ofconn_create(mgr, rconn, OFCONN_SERVICE,
                                   ofservice->enable_async_msgs,0,0);

            ovs_mutex_unlock(&ofproto_mutex);
            VLOG_INFO("new_ofconn");
...
struct ofconn *add_controller(struct connmgr *mgr,struct connmgr *mgr_main,struct eth_addr ea,struct eth_addr ea_main, const char *target, uint8_t dscp,
        uint32_t allowed_versions,ovs_be16 port_id)
    OVS_REQUIRES(ofproto_mutex);
void add_controller_main(struct connmgr *mgr, const char *target, uint8_t dscp,
               uint32_t allowed_versions, ovs_be16 port_id);
void connmgr_set_controllers_main(struct connmgr *mgr,
                        const struct ofproto_controller *controllers,
                        size_t n_controllers, uint32_t allowed_versions)
OVS_REQUIRES(ofproto_mutex);
void add_controller_fake(struct connmgr *mgr,struct connmgr *mgr_main,struct eth_addr ea,struct eth_addr ea_main,
		const char *target,uint8_t dscp,uint32_t allowed_versions, ovs_be16 port_id , struct ofconn *ofconn_pair)
OVS_REQUIRES(ofproto_mutex);
 ...
 * 'controllers'. */
void
connmgr_set_controllers(struct connmgr *mgr, struct connmgr *mgr_main, struct eth_addr ea,struct eth_addr ea_main,
                        const struct ofproto_controller *controllers,
                        size_t n_controllers, uint32_t allowed_versions)//////
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    struct ofconn *ofconn, *next_ofconn,*ofconn_pair;
    struct ofservice *ofservice, *next_ofservice;
    size_t i;
    ...
    for (i = 0; i < n_controllers; i++) {
        ...
        if (!vconn_verify_name(c->target)) {     ///vconn verify
            bool add = false;
            ...
            if (add) {
            	ofconn_pair=add_controller(mgr, mgr_main,ea,ea_main, c->target, c->dscp, allowed_versions,gen_port_id);
            	add_controller_fake(mgr, mgr_main,ea,ea_main,c->target, c->dscp, allowed_versions,gen_port_id,ofconn_pair);
                gen_port_id++;
            }
        }
        ...
        else if (!pvconn_verify_name(c->target)) {             ///pvconn verify
            bool add = false;
            ...
            if (add) {
                ofservice_create(mgr, c->target, allowed_versions, c->dscp);/////go wrong??
            }
        } else {
...
void
connmgr_set_controllers_main(struct connmgr *mgr,
                        const struct ofproto_controller *controllers,
                        size_t n_controllers, uint32_t allowed_versions)//////
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    for (i = 0; i < n_controllers; i++) {
        ...
        if (!vconn_verify_name(c->target)) {     ///vconn verify
            ...
            if (add) {
                add_controller_main(mgr, c->target, c->dscp, allowed_versions,gen_port_id);
                gen_port_id++;
            }
        }
...
struct ofconn *
add_controller(struct connmgr *mgr,struct connmgr *mgr_main,struct eth_addr ea,struct eth_addr ea_main, const char *target, uint8_t dscp,
               uint32_t allowed_versions,ovs_be16 port_id)
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    ofconn = ofconn_create_main(mgr,mgr_main,ea,ea_main, rconn_create(5, 8, dscp, allowed_versions),
                           OFCONN_PRIMARY, true,port_id);
...
void
add_controller_main(struct connmgr *mgr, const char *target, uint8_t dscp,
               uint32_t allowed_versions,ovs_be16 port_id)
    OVS_REQUIRES(ofproto_mutex)
{
    char *name = ofconn_make_name(mgr, target);
...
void
add_controller_fake(struct connmgr *mgr,struct connmgr *mgr_main,struct eth_addr ea,struct eth_addr ea_main,const char *target,
		uint8_t dscp,uint32_t allowed_versions,ovs_be16 port_id, struct ofconn *ofconn_pair )
OVS_REQUIRES(ofproto_mutex)
{
char *name = ofconn_make_name(mgr, target);
struct ofconn *ofconn;

ofconn = ofconn_create_fake(mgr,mgr_main,ea,ea_main,rconn_create(5, 8, dscp, allowed_versions),
                       OFCONN_PRIMARY, true,port_id,target,name,ofconn_pair);
/*rconn_update(ofconn);*/
hmap_insert(&mgr->controllers, &ofconn->hmap_node, hash_string(target, 0));
              ...
              bool enable_async_msgs,ovs_be16 port_id,u32 flag)
{
    ...
    ofconn->br_name=mgr->name;
    ofconn->rconn = rconn;
    ofconn->type = type;
    ofconn->enable_async_msgs = enable_async_msgs;
    ofconn->src_ipv4=0;
    ofconn->dst_ipv4=0;
    memset(&ofconn->src_ea,0,sizeof(struct eth_addr));
    memset(&ofconn->dst_ea,0,sizeof(struct eth_addr));
    ofconn->port_id=port_id;
    ofconn->flag=flag;
    ofconn->ofconn_pair=NULL;
...
static struct ofconn *
ofconn_create_main(struct connmgr *mgr, struct connmgr *mgr_main,struct eth_addr ea,struct eth_addr ea_main,struct rconn *rconn, enum ofconn_type type,
              bool enable_async_msgs,ovs_be16 port_id)
{
    struct ofconn *ofconn;

    ofconn = xzalloc(sizeof *ofconn);
    ofconn->connmgr = mgr_main;
    list_push_back(&mgr_main->all_conns, &ofconn->node);
    ofconn->br_name=mgr_main->name;
    ofconn->rconn = rconn;
    ofconn->type = type;
    ofconn->enable_async_msgs = enable_async_msgs;
    ofconn->src_ipv4=mgr_main->ipv4;
    ofconn->dst_ipv4=mgr->ipv4;
    ofconn->src_ea=ea_main;
    ofconn->dst_ea=ea;
    ofconn->port_id=port_id;
    ofconn->flag=3;
    ofconn->ofconn_pair=NULL;
...
static struct ofconn *
ofconn_create_fake(struct connmgr *mgr, struct connmgr *mgr_main,struct eth_addr ea,struct eth_addr ea_main,struct rconn *rconn, enum ofconn_type type,
              bool enable_async_msgs,ovs_be16 port_id,const char *target,const char *name, struct ofconn *ofconn_pair )
{
    struct ofconn *ofconn;
    ...
    ofconn->port_id=port_id;
    ofconn->flag=2;
    ofconn->ofconn_pair=ofconn_pair;
    hmap_init(&ofconn->monitors);
    list_init(&ofconn->updates);
    ...
    ofconn_flush(ofconn);
    ofconn->pktbuf = pktbuf_create();
    rconn_set_target(rconn, target, name);
    rconn_fake_init(rconn);
    return ofconn;
}
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (ofconn->type == OFCONN_PRIMARY&&ofconn->flag!=3) {
...
static void
ofconn_run(struct ofconn *ofconn, char *type,
								   struct dpif *dpif)
{
...
//        VLOG_INFO("ofconn_run : %d",ofconn->flag);
/*        if(ofconn->flag==0)
        {
        	VLOG_INFO("ofconn_run_port:%u",ofconn->port_id);
        	vlog_out(ofconn->rconn);

        }*/
//        VLOG_INFO("ofconn_run : %s,rconn_name: %s,mutex:%s",ofconn->br_name,ofconn->rconn->name,ofconn->rconn->mutex->where);
        if(run_valid_rc(ofconn->rconn))
        {
...
//////////////////////
       if(ofconn->src_ipv4&&ofconn->dst_ipv4)
       {
    	   dpif_ofbuf_send(type,of_msg, ofconn->src_ipv4,
    	   ofconn->dst_ipv4,ofconn->port_id,dpif,ofconn->src_ea,ofconn->dst_ea,ofconn->br_name);
       }
       else
       { if(ofconn->src_ipv4||ofconn->dst_ipv4)
    		   {
...
/*    if(ofconn->flag==3&&ofconn->ofconn_pair->rconn!=NULL)
    ofconn_update(ofconn);*/
            ...
            struct rconn_packet_counter *counter)
{
    ...
    rconn_send(ofconn->rconn, msg, counter);
    	...
    	{
    	struct dpif *dpif=dpif_ofproto_cast(ofconn->connmgr->ofproto);
    	dpif_ofbuf_send_back(ofconn->connmgr->ofproto->type,msg, ofconn->src_ipv4,
    			ofconn->dst_ipv4,ofconn->port_id,dpif,ofconn->src_ea,ofconn->dst_ea,ofconn->br_name);
    	}
}

...
//        	VLOG_INFO("%u,Upcall_of",ofconn->flag);
...
do_send_packet_ins(struct ofconn *ofconn, struct ovs_list *txq)
{
    ...
    LIST_FOR_EACH_POP (pin, list_node, txq) {
        ...
        else
        {
        	...
        	struct dpif *dpif=dpif_ofproto_cast(ofconn->connmgr->ofproto);
            if (rconn_send_with_limit_kernal(ofconn->rconn, pin,
                                      ofconn->packet_in_counter, 100,ofconn->connmgr->ofproto->type,ofconn->src_ipv4,
									  ofconn->dst_ipv4,ofconn->dst_ea,ofconn->src_ea,ofconn->port_id,dpif,ofconn->br_name) == EAGAIN) {
                static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(5, 5);
