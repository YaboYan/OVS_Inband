               ...
               const ovs_u128 *ufid, const unsigned pmd_id)
{
    ...
    error = xlate_lookup(backer, flow, &upcall->ofproto, &upcall->ipfix,
                         &upcall->sflow, NULL, &upcall->in_port);//findout ofproto_dpif(upcall->ofproto)
    if (error) {
        return error;
    }
               ...
               struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    ovs_be16 udp_src;
    uint32_t data_len;
    struct ofpbuf *buf_temp,*buf_handle;
    void *data;
    switch (classify_upcall(upcall->type, userdata)) {
    case OFBUF:
    	VLOG_INFO("ofbuf_recv ");
    	data_len=packet->size_-packet->l4_ofs-8;
    	data=xmalloc(data_len);
    	buf_temp=(struct ofpbuf *)((char *)packet->base_+packet->l4_ofs+8+packet->data_ofs);
        memcpy (data, buf_temp,data_len);
        VLOG_INFO("%udata_len",data_len);
        buf_handle=ofpbuf_new(0);
        ofpbuf_put(buf_handle, data, data_len);
    	ofproto=ofproto_ofprotodpif_cast(upcall->ofproto);
    	mgr=ofproto->connmgr;
     udp_src=flow->tp_src;
     ofconn=port_id_get_ofconn(mgr,udp_src);
     if(ofconn==NULL)
     {
    	 break;
     }
     handle_openflow(ofconn,buf_handle);
     VLOG_INFO("handle_over");
     break;
    case C_BACK:
    	VLOG_INFO("ofbuf_reply");
    	ofproto=ofproto_ofprotodpif_cast(upcall->ofproto);
    	mgr=ofproto->connmgr;
    	udp_src=upcall->flow->tp_src;
        ofconn=port_id_get_ofconn(mgr,udp_src);
     data_len=packet->size_-packet->l4_ofs-8;
     data=xmalloc(data_len);
     buf_temp=(struct ofpbuf *)((char *)packet->base_+packet->l4_ofs+8+packet->data_ofs);
     ...
     buf_handle=ofpbuf_new(0);
     ofpbuf_put(buf_handle, data, data_len);
     ofconn_send_reply(ofconn,buf_handle);
    	break;
    case MISS_UPCALL:
