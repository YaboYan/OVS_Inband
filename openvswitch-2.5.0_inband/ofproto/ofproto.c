...
ofproto_create(const char *datapath_name, const char *datapath_type,
               struct ofproto **ofprotop,__be32  ipv4)
{
    ...
    *ofprotop = NULL;
    ...
    ofproto->ipv4=ipv4;
    ofproto->connmgr = connmgr_create(ofproto, datapath_name, datapath_name, ipv4);
    ...
    error = ofproto->ofproto_class->construct(ofproto);///init_dpif
    if (error) {
...
void
ofproto_set_controllers(struct ofproto *p, struct ofproto *p_main, struct eth_addr ea,struct eth_addr ea_main,
                        const struct ofproto_controller *controllers,
                        size_t n_controllers, uint32_t allowed_versions)
{
    connmgr_set_controllers(p->connmgr, p_main->connmgr,ea,ea_main,controllers, n_controllers,
                            allowed_versions);
}
void
ofproto_set_controllers_main(struct ofproto *p,
                        const struct ofproto_controller *controllers,
                        size_t n_controllers, uint32_t allowed_versions)
{
    connmgr_set_controllers_main(p->connmgr,controllers, n_controllers,
                            allowed_versions);
}
void
...
ofproto_run(struct ofproto *p)
{
    ...
    struct dpif *dpif = dpif_ofproto_cast(p);
    connmgr_run(p->connmgr,p->type,dpif);
...
//    if(ofconn_flag(ofconn)==2)
    VLOG_INFO("%d,enum_type,ofconn:%u",type,ofconn_flag(ofconn));
    switch (type) {
        /* OpenFlow requests. */
