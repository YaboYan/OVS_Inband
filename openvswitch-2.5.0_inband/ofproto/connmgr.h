...
struct connmgr *connmgr_create(struct ofproto *ofproto,
                               const char *dpif_name, const char *local_name,__be32  ipv4);
void connmgr_destroy(struct connmgr *);

void connmgr_run(struct connmgr *mgr, char *type,
								struct dpif *dpif);
...
void connmgr_set_controllers(struct connmgr *mgr, struct connmgr *mgr_main, struct eth_addr ea,struct eth_addr ea_main,
...
struct ofconn * port_id_get_ofconn(struct connmgr *mgr,ovs_be16 tcp_src);
...
void ofmonitor_compose_refresh_updates(struct rule_collection *rules,
    ...
    OVS_REQUIRES(ofproto_mutex);
u32 ofconn_flag(struct ofconn *ofconn);
#endif /* connmgr.h */
